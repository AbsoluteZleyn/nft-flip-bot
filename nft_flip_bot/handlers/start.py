...
WELCOME = (
    "👋 Привет! Я помогаю флипперить Telegram-NFT-подарки.\n\n"
    "Команды:\n"
    "• Отправь ссылку на лот или ID токена — я проанализирую (модель, "
    "фон, узор, редкости, прогноз resale).\n"
    ...
    "• /scan_on — включить уведомления (по умолчанию раз в минуту)\n"
    "• /scan_off — выключить уведомления\n"
    "• /scan_now — прогон скана прямо сейчас\n"
    "• /range <min> <max> — задать персональный диапазон цен в TON\n"
    "• /range off — сбросить диапазон к глобальным значениям\n"
    "• /history <UUID> — статистика по комбо (модель + фон)\n"
    "\n"
